...
class WCFC_Api {
	...
	public function __construct() {
		...
		if ( isset( $wcfc_settings['mode'] ) && 'sandbox' == $wcfc_settings['mode'] ) :
			$this->mode = 'sandbox';
			$this->url  = 'http://sandbox.freightcenter.com';
		endif;
...
return array();
		...
		try {
			...
			$client = new SoapClient( $this->url . '/' . $this->api_version . '/carriers.asmx?wsdl', array(
				'trace' 			=> 1,
				'exceptions' 		=> 1,
				'encoding' 			=> 'UTF-8',
			));
	...
	public function get_rates( $args = array(), $post_data ) {
		...
		try {
			...
			$client = new SoapClient( $this->url . '/' . $this->api_version . '/rates.asmx?wsdl', array(
				'trace' 			=> 1,
				'exceptions' 		=> 1,
				'encoding' 			=> 'UTF-8',
			));
	...
	public function cancel_shipment( $order_id, $shipment_id, $reason = '' ) {
		...
		try {
			...
			$client = new SoapClient( $this->url . '/' . $this->api_version . '/Shipments.asmx?wsdl', array(
				'trace' 			=> 1,
				'exceptions' 		=> 1,
				'encoding' 			=> 'UTF-8',
			));
